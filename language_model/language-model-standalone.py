...
def connect_to_redis_server(redis_ip, redis_port):
    try:
        # logging.info("Attempting to connect to redis...") with password
        redis_conn = redis.Redis(host=redis_ip, port=redis_port, socket_connect_timeout=10)
        redis_conn.ping()
    except redis.exceptions.ConnectionError:    
        logging.warning("Can't connect to redis server (ConnectipingonError).")
        return
    else:
